...
const baseurl = `http://0.0.0.0:${process.env.OS === 'Android' ? '4723' : '4725'}/wd/hub`;
let pageObjectActions = require(`../lib/${process.env.OS === 'Android' ? 'pageObjectAndroid' : 'pageObjectIos'}`);
pageObjectActions = new pageObjectActions();

//Custom methods for components
exports.startRecordingScreen = function () {
	return new Promise((resolve, reject) => {
		const endpointGetSession = '/sessions';
		let options = {
			json: false,
			followRedirect: true,
			headers: {
		...
		// console.log(`GET: ${baseurl}${endpointGetSession}`.cyan);
		got
			.get(`${baseurl}${endpointGetSession}`)
			.then((sessionDetails) => {
				sessionDetails = JSON.parse(sessionDetails.body);
				global.wdSessionId = sessionDetails.value[0].id;
				this.getWindowSize().then((size) => {
					options = {
						followRedirect: true,
						headers: { accept: '*/*', 'Accept-Encoding': 'gzip, deflate, br', Connection: 'keep-alive' },
						body: `{"options": {"videoType": "libx264", "videoScale": "${size.width % 2 === 0 ? size.width : size.width - 1}:${size.height % 2 === 0 ? size.height : size.height - 1}", "videoFps": "20"}}`,
					};
					const endpointStartRecording = `/session/${wdSessionId}/appium/start_recording_screen`;
					console.log(`POST: ${baseurl}${endpointStartRecording}`.blue);
					got.post(`${baseurl}${endpointStartRecording}`, options).then((endpointStartRecordingResponse) => {
						console.log('* Video Record started with sessionId:'.yellow, JSON.parse(endpointStartRecordingResponse.body).sessionId);
						resolve(this);
					});
				});
			})
...
exports.stopRecordingScreen = function (videoName, shouldSave) {
	return new Promise((resolve) => {
		...
		const endpointStopRecording = `/session/${wdSessionId}/appium/stop_recording_screen`;
		console.log(`POST: ${baseurl}${endpointStopRecording}`.blue);
		got
			.post(`${baseurl}${endpointStopRecording}`, options)
			.then((endpointStopRecordingResponse) => {
				if (shouldSave === true) {
					endpointStopRecordingResponse = JSON.parse(endpointStopRecordingResponse.body);
					const videobase64 = endpointStopRecordingResponse.value;
					fs.writeFile(`${process.env.PWD}/reports/${videoName}.mp4`, videobase64, 'base64', function (err) {
						if (err) resolve(err);
						else resolve(`${process.env.PWD}/reports/${videoName}.mp4`);
					});
				} else {
					endpointStopRecordingResponse = '';
					resolve(this);
				}
			})
			.catch((err) => {
...
exports.deleteapp = function () {
	const appPackageAndroid = '';
	if (process.env.OS === 'Android') return this.removeApp(appPackageAndroid);
	else return this;
};
